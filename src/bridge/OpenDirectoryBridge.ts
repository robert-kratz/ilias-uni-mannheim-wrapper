...
ipcMain.handle('open-directory', async (event, directoryId: string): Promise<OpenDirectoryResponse> => {
    return new Promise((resolve, reject) => {
        ...
        if (!directory) {
            ...
            const course = db.prepare('SELECT * FROM courses WHERE id = ? AND userId = ?').get(directoryId, userId) as
                | Course
                | undefined;
        ...
        const files = db.prepare('SELECT * FROM files WHERE parentId = ? AND userId = ?').all(directoryId, userId) as
            | File[]
            | undefined;
        ...
        if (directory.parentId) {
            parentDirectory = db.prepare('SELECT * FROM directories WHERE id = ?').get(directory.parentId) as Directory;
        }
