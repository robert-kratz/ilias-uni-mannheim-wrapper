 ...
 */
const fetchUserDataFromHtml = async (): Promise<string | null> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const response = await axios.get(
        ...
        {
          // Add headers like authentication tokens if needed
          headers: {
      ...
      const emailElement = $("#usr_email");
      const email = emailElement.length ? emailElement.val() : null;

      // Return the fetched email
      resolve(email as string);
    } catch (error) {
    ...
    try {
      ...
      folderBlocks.each((_, block) => {
        ...
        items.each((_, item) => {
          ...
          const descriptionElement = $(item).find(
            ".ilListItemSection.il_Description"
          );
...
const scrapePage = async (sessionCookie: string) => {
  ...
  const toScrapeCourse = scrapedGroups.filter(
    (year) => year.year === "HWS 2024"
  );
    ...
    year.courses.forEach((course) => {
      ...
      scrapeJobs.push(
        new Promise(async (resolve, reject) => {
          ...
          const scrapedFolders = await scrapeFolderDirectoriesFromHtml(
            htmlContentCourse
          );
          ...
          scrapedFolders.forEach((folder) => {
            console.log(
              `(DIR) - ${folder.title} (${folder.description} - ${folder.link})`
            );
          });

          const scrapedFiles = await scrapeCourseFilesFromHtml(
            htmlContentCourse
          );
          ...
          scrapedFiles.forEach((file) => {
            console.log(
              `(FILE) - ${file.title} (${file.description} - ${file.link})`
            );
          });
